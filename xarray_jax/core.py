...
from typing import Any, Hashable, Mapping, Optional, TypeVar  # pylint: disable=deprecated-class
...
JAX_COORD_ATTR_NAME = '_jax_coord'
...
def assign_coords(
  ...
  for name, coord in jax_coords.items():
    ...
    coord.attrs[JAX_COORD_ATTR_NAME] = True
...
def get_jax_coords(x: DatasetOrDataArray) -> Mapping[Hashable, Any]:
  return {
      ...
      if coord_var.attrs.get(JAX_COORD_ATTR_NAME, False)}
